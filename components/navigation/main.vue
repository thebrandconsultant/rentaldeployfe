<template>
  <nav class="absolute w-full py-3 px-6 bg-transparent z-10">
    <div class="flex items-center justify-between">

      <!-- Header logo -->
      <div>
        <img class="h-8" src="/whiteLogo.png"/>
      </div>
      ...
      <!-- Drawer Menu -->
      <aside class="p-5 px-1 transform top-0 left-0 w-5/6 bg-gray-700 fixed h-full overflow-auto ease-in-out transition-all duration-300 z-30" :class="isOpen ? 'translate-x-0' : '-translate-x-full'">
        
        <div class="close">
          <button class="text-white absolute top-0 right-0 mt-4 mr-4" @click=" isOpen = false">
            <svg 
              class="w-6 h-6"
              fill="none" stroke-linecap="round" 
              stroke-linejoin="round" stroke-width="2"
        ...
        <span @click="isOpen = false" class="flex w-full items-center px-1">
          <img class="h-12" src="/whiteLogo.png"/>
        </span>

        <ul class="divide-y font-sans p-4 divide-gray-50 text-white">
          ...
          <li><a href="#" @click="isOpen = false" class="my-8 w-full text-center font-semibold cta inline-block bg-skin-accent hover:bg-gray-900 px-6 py-2 rounded text-black">Sign Up</a></li>
        </ul>

        <div class="follow">
          <p class="italic font-sans text-sm p-4 text-white text-center">follow us:</p>
          <div class="social flex space-x-5 mt-4 mx-auto w-3/4 ">
            <a href="#">
              <svg 
